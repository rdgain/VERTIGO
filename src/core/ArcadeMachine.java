...
public class ArcadeMachine {
	...
	public static double[] runOneExpGame(JFrame frame, JPanel gamePanel, String game_file, String level_file, boolean visuals,
										 String agentNames, String actionFile, String evoFile, int randomSeed, int playerID, ParameterSet params)
	{
		...
		for (int i = 0; i < no_players; i++) {
			...
			if (no_players > 1) {
				...
				//single player
				players[i] = ArcadeMachine.createExpPlayer(names[i], actionFile, evoFile, toPlay.getObservation(), randomSeed, humans[i], params);
			}
	...
	private static AbstractPlayer createExpPlayer(String playerName, String actionFile, String evoFile,
												  StateObservation so, int randomSeed, boolean isHuman, ParameterSet params)
	{
		...
		try{
			...
			if(player != null)
				player.setup(actionFile, evoFile, randomSeed, isHuman, params);
